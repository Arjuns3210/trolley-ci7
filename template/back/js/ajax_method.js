			...
			success: function(data) {
				...
				if(type == 'first'){
					...
					if (LAST_KNOWN_PAGE>0){
                        ...
                        $('#demo-table').bootstrapTable('nextPage',LAST_KNOWN_PAGE);
					} else {
					...
					$('#demo-table img').each(function() {
						if($(this).attr('src') !== ''){
							if($(this).data('im') !== 'fb'){
								// commented by satesh 24_01_2019 :: Start 
								// reason : its appending at the end of google drive link
						    	//$(this).attr('src', $(this).attr('src')+'?random='+new Date().getTime());
						    	// commented by satesh 24_01_2019 :: End
							}
						}
					});
				} else if(type=='form') {
...
//				console.log(e)
	...
	function ajax_set_list(extra){
        ...
        function ajax_redirect(value){
		window.location = base_url+''+user_type+'/'+value;
	}

	function ajax_set_full(type,title,noty,form_id,id){
		//full_form(title,noty,form_id);
//                  console.log('Inside ajax_method.js ajax_set_full type has ::'+type);
//                  console.log('Inside ajax_method.js ajax_set_full id has ::'+id);
//                  console.log('Inside ajax_method.js ajax_set_full noty has ::'+noty);
//                  console.log('Inside ajax_method.js ajax_set_full form_id has ::'+form_id);
	...
	function delete_confirm(id,msg){
        ...
        function msg_confirm(id,msg,path_to,success_msg){
		msg = '<div class="modal-title">'+msg+'</div>';
		bootbox.confirm(msg, function(result) {
			if (result) {
				ajax_load(base_url+''+user_type+'/'+module+'/'+path_to+'/'+id,'list','delete');
				$.activeitNoty({
					type: 'success',
					icon : 'fa fa-check',
					message : success_msg,
	...
	function other_confirm(func,id,msg,noty){
        ...
        function modal_on_confirm(type,title,noty,form_id,id,confirm_msg){
		msg = '<div class="modal-title">'+confirm_msg+'</div>';
		bootbox.confirm(msg, function(result) {
			if (result) {
                            ajax_modal(type,title,noty,form_id,id);
			}else {
				$.activeitNoty({
					type: 'danger',
					icon : 'fa fa-minus',
...
//                    console.log(e)
	...
	function form_submit(form_id,noty,e){
		...
		var form = $('#'+form_id);
                //alert("inside ajax_method form_submit form :: "+form);
		var can = '';
		if(!extra){
			var extra = '';
		}
		...
		form.find(".required").each(function(){
            ...
            if(here.val() == ''){
                ...
                var topp = 100;
               if(form_id == 'sales_add' || form_id == 'product_add' || form_id == 'product_edit' || form_id == 'supplier_add' || form_id == 'supplier_edit'){
                } else {
	                //updated by Ritesh
			...
			if(form_id !== 'vendor_pay'){
				$.ajax({
					...
					success: function(res) {
                                            ...
                                            //added by ritesh to display errors: start
                                                if(res!==''){
                                                    if($('.modal.in, .modal.show').length){
                                                        //To Show error in model like in category 
                                                        modalErrorMsg(res);
                                                    }else{
                                                        // On screen error 
                                                         $.activeitNoty({
						...
						buttonp.removeClass('disabled');
                                            ...
                                            //added by ritesh to display errors: end
...
//						console.log(e)
	...
	if(typeof set_switchery != 'function'){
		window.set_switchery = function(){
			...
			//added by sagar for status toggle  for customers
					...
					changeCheckbox.onchange = function() {
					  //alert($(this).data('id'));
					  ajax_load(base_url+''+user_type+'/'+module+'/user_publish_set/'+$(this).data('id')+'/'+changeCheckbox.checked,'customerlist','others');
					  if(changeCheckbox.checked == true){
						$.activeitNoty({
							type: 'success',
							icon : 'fa fa-check',
	...
	//data-bb-handler="success"
        ...
        function addSwitchery(fieldName,para,success_msg,error_msg){
            $("."+fieldName).each(function(){
		new Switchery(document.getElementById(fieldName+'_'+$(this).data('id')), {color:'rgb(100, 189, 99)', secondaryColor: '#cc2424', jackSecondaryColor: '#c8ff77'});
		var changeCheckbox = document.querySelector('#'+fieldName+'_'+$(this).data('id'));
//                console.log(changeCheckbox);
		changeCheckbox.onchange = function() {
		  ajax_load(base_url+'index.php/'+user_type+'/'+module+'/'+para+'/'+$(this).data('id')+'/'+changeCheckbox.checked,'','');
		  if(changeCheckbox.checked == true){
			$.activeitNoty({
				type: 'success',
				icon : 'fa fa-check',
        ...
        //added by sagar : alert msg in ajax-modal form  START 5-july
         function modalErrorMsg(msg=''){
            if( msg != ''){
               var success_msg = '<div id="error_msg_id" class="alert alert-danger" role="alert">';
                   success_msg += '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>';
                   success_msg += '<p>'+msg+'</p>';
                   success_msg += '</div>';
               $('.bootbox-body').before(success_msg);
               setTimeout(function(){
                   $('#error_msg_id').remove();
               }, 4000);
    ...
    //added by sagar : alert msg in ajax-modal form  END 5-july