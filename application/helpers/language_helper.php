 ...
 */
	...
	function asset_url(){
		return 'http://developers.mypcot.com/themallv1.0/';
	}
...
if ( ! function_exists('get_converted_currency'))
{
    function get_converted_currency($price=null ,$curr_code = '1', $conversion_sale_rate = 0){
        $CI=& get_instance();
        $CI->load->database();
        $converted_price = 0;
        if($price != null && is_numeric($price)){
            if($curr_code == '1'){
                $converted_price = round($price, 2);
            }else if($curr_code == '2'){
                $currency_conversion_rate = 0;
                if(empty($conversion_sale_rate)) {
                 $currency_conversion_rate =  $CI->db->get_where('general_settings',array('type' => 'currency_conversion'))->row()->value;
                 if(empty($currency_conversion_rate)){
                     $currency_conversion_rate = 0; //1
                 }
                }else{
                    $currency_conversion_rate = $conversion_sale_rate;
                }
                $converted_price = $price * $currency_conversion_rate;
...
if (!function_exists('maskMobileNumber')) {
    function maskMobileNumber($cardNumber = '') {
        $cardNumberLength = strlen($cardNumber);
        $returnP1 = substr($cardNumber, 0, 4);
        $returnP2 = substr($cardNumber, -2);
        $paddingLength = $cardNumberLength - 2;
...
if (!function_exists('refreshedImage')) {
    function refreshedImage(){
        $default_start_time = (date('Y-m-d 12:00:00'));
        $default_end_time = (date('Y-m-d 24:00:00'));
        $current_time = (date('Y-m-d H:i:s') );
//        $current_time_a = (date('Y-m-d H:i:s') );
//        $current_time =  date("d-m-Y H:i:s", strtotime("$current_time_a + 12 hours"));
        if(strtotime($default_start_time) < strtotime($current_time) && strtotime($default_end_time) > strtotime($current_time) ){
            return strtotime($default_end_time);
        }else{
            return strtotime($default_start_time);
        }
