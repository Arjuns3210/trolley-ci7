	...
	<!--Demo script [ DEMONSTRATION ]-->
	<script src="<?php echo base_url(); ?>template/back/js/ajax_method.js?d=<?php echo time();?>"></script>
    ...
    <script type="text/javascript">
        <?php // changed by dev--Start ?>
        <?php if($this->session->userdata('login_type')=='warehouse' || $this->session->userdata('login_type')=='admin'){ ?>
    	setInterval(sale_check, 300000);
        <?php } ?>
    	<?php // changed by dev--End ?>
				...
				success: function(data) {
					if(data == 'yah!good'){}
						...
						location.replace('<?php echo base_url(); ?>index.php/<?php echo $this->session->userdata('title'); ?>');
                                                <?php */ ?>    
						location.replace('<?php echo base_url(); ?>index.php/<?php echo $this->session->userdata('login_type'); ?>');
                                            <?php //changed by dev --End ?>
					}
				},
				error: function(e) {
