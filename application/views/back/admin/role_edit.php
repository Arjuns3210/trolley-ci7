		...
		?>
            <div class="panel-body">
                <div class="form-group margin-top-15">
                    ...
                    <div class="col-sm-6">
                        <input type="text" name="name" readonly="" id="demo-hor-1" value="<?php echo $row['name']; ?>" class="form-control required" placeholder="<?php echo translate('name'); ?>" >
                    </div>
                </div>
                            ...
                            <?php
                                ...
                                foreach ($all_permissions as $key => $value) {
                                    if($value['parent_status'] == 'parent') {
                                        array_push($sortedPermissions,$value);
                                    }else{
                                        $keyPosition = array_search($value['parent_status'], array_column($sortedPermissions, 'permission_id'));
                                        if(is_numeric($keyPosition)){
                                            array_splice($sortedPermissions, $keyPosition+1, 0, '');
                                            $sortedPermissions[$keyPosition+1] = $value;
                                        }
                                      
                                    }
                                }
                                
                                foreach($sortedPermissions as $row1){
                                    if($row1['to_be_considered']=='Yes'){
                            ?>
                            <tr>
                                <td>
                ...
                <div class="row">
                    ...
                    <div class="col-md-1">
                        <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right" 
                            onclick="form_submit('role_edit','<?php echo translate('successfully_edited!'); ?>');proceed('to_add');" >
